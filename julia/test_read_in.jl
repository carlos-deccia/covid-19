...
Data_matrix = [zeros(rows_entries,Ntime_elements) for _ in 1:3]
...
for i=1:rows_entries
    for j=5:col_time
        Data_matrix[1][i,j-4] = myData_conf[i,j]
        Data_matrix[2][i,j-4] = myData_deat[i,j]
        Data_matrix[3][i,j-4] = myData_reco[i,j]
    end
end
